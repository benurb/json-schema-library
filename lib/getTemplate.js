...
let cache;
function resolveRef(core, schema, ignore = false) {
    if (schema.$ref == null) {
        return schema;
    }
    ...
    if (!ignore && (cache[id] == null || cache[id] < settings.GET_TEMPLATE_RECURSION_LIMIT)) {
        cache[id] = cache[id] || 0;
        cache[id] += 1;
        return core.resolveRef(schema);
    }
    console.log("! abort ref resolution");
    return schema;
}
...
function getTemplate(core, data, schema) {
    ...
    // console.log("getTemplate", schema, data);

    // resolve $ref references
    schema = resolveRef(core, schema, data != null);
    ...
    // eslint-disable-next-line no-use-before-define
    const res = TYPE[schema.type](core, schema, data);
...
const TYPE = {
    ...
    "array": (core, schema, data) => {
        ...
        console.log("!!! simply return", data, schema);
        return data;
    }
};
